...
angular.module(_SERVICES_).service('importServiceBkup', function ($rootScope,
                                                              $q,
                                                              $log,
                                                              $timeout,
                                                              serverAPI,
  ...
  function getSignedUrl(importObj) {
    ...
    if (importObj.photoObj.url) {
      ...
      serverAPI.getSignedImageUrl(importObj.galleryId, importObj.photoObj.id, { timeout: importObj.cancelAllHttpRequestsPromise.promise }).then(function (result) {
    ...
    if (photoObjects && photoObjects.length) {
      ...
      angular.forEach(photoObjects, function (photoObj) {
        var importObject = {
          ...
          'errors': errors,
          'status': status,
          'cancelAllHttpRequestsPromise': test
        };
